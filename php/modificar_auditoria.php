...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_update = "UPDATE auditoria SET estado = ?, motivo = ?, analista = ?, responsable = ?, observacion = ?, fecha_respuesta = CURRENT_TIMESTAMP WHERE id = ?";
    ...
    if (mysqli_stmt_affected_rows($stmt_update) > 0) {
        ...
        $sql_insert_historial = "INSERT INTO historial_auditoria (id_original, estado, motivo, analista, responsable, observacion, fecha_respuesta, usuario_modificacion) VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, ?)";
    ...
    try {
        ...
        if (isset($_FILES['archivos']) && !empty($_FILES['archivos']['name'][0])) {
            ...
            for ($i = 0; $i < count($_FILES['archivos']['name']); $i++) {
                ...
                $stmt = $conn->prepare("INSERT INTO archivos_auditoria (nombre_archivo, tipo_archivo, datos_archivo, id_pqr) VALUES (?, ?, ?, ?)");
