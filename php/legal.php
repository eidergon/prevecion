...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmtUpdate = $conn->prepare("UPDATE datos SET desicion_legal = ?, fecha_desicion_legal = CURRENT_TIMESTAMP WHERE id = ?");
    ...
    try {
        ...
        if (isset($_FILES['legal']) && !empty($_FILES['legal']['name'][0])) {
            ...
            for ($i = 0; $i < count($_FILES['legal']['name']); $i++) {
                ...
                $stmt = $conn->prepare("INSERT INTO legal (nombre_archivo, tipo_archivo, datos_archivo, id_pqr) VALUES (?, ?, ?, ?)");
